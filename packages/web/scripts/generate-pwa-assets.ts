...
const ICON_CONFIGS: IconConfig[] = [
  ...
  { name: 'icon-maskable-192', size: 192, isMaskable: true, emoji: '✨', backgroundColor: COLORS.sage400 },
  { name: 'icon-maskable-512', size: 512, isMaskable: true, emoji: '✨', backgroundColor: COLORS.sage400 },
...
async function generateScreenshotPlaceholders() {
  ...
  for (const screenshot of screenshots) {
    ...
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText(`${screenshot.label}`, screenshot.width / 2, screenshot.height / 2 - 20);
    ctx.font = '16px system-ui, sans-serif';
    ctx.fillStyle = COLORS.sage500;
    ctx.fillText(`${screenshot.width}x${screenshot.height}`, screenshot.width / 2, screenshot.height / 2 + 20);
    ctx.fillText('⚠ Replace with actual screenshot', screenshot.width / 2, screenshot.height / 2 + 50);
  ...
  console.log('⚠️  IMPORTANT: Replace these with actual app screenshots for production.\n');

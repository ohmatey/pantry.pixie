...
├── onboarding.tsx         → /onboarding (with AuthGuard)
├── invite/
│   └── [code].tsx         → /invite/:code (dynamic route)
...
- **Parentheses = organizational grouping (no URL impact)**
  - `(app)/chat.tsx` → `/chat` (NOT `/app/chat`)
  - Use for grouping related routes with shared layouts/guards

## ✅ Benefits Over Manual React Router
...
3. Update route generator:
   ...
   children: [
     ...
     { path: 'profile', element: withSuspense(ProfilePage) }, // ← Add here
   ]
   ```
...
### ❌ Why Not Bun's File-System Router?
...
*Generated by Pantry Pixie routing migration (2026-02-15)*

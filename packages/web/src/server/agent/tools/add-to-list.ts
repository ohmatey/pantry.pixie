...
export function createAddToListTool(homeId: string, scopedListId?: string | null) {
  return tool({
    description:
    ...
    parameters: z.object({
      items: z.array(
        z.object({
          name: z.string().describe("Item name (e.g., 'chicken breast', 'jasmine rice')"),
          quantity: z.number().optional().describe("How many/much to buy (e.g., 2, 500)"),
          unit: z.string().optional().describe("Unit (e.g., 'g', 'kg', 'pieces', 'bunch', 'can')"),
          notes: z.string().optional().describe("Extra context (e.g., 'organic', 'for green curry')"),
        })
      ).describe("Items to add to the list with optional quantity, unit, and notes"),
      listName: z.string().optional().describe("Name of the list (optional, defaults to scoped list or 'Quick Items')"),
    }),
    execute: async ({ items, listName }) => {
      ...
      if (scopedListId) {
        ...
        const scopedList = await groceryListsService.getList(homeId, scopedListId);
        if (scopedList) {
          listId = scopedList.id;
        } else {
          // Fallback if scoped list not found
          const defaultList = await groceryListsService.getOrCreateDefaultList(homeId);
        ...
        if (found) {
          ...
          const newList = await groceryListsService.createList(homeId, { name: listName });
          listId = newList.id;
        }
      } else {
        // Use default list
      ...
      for (const itemData of items) {
        ...
        const item = await groceryListsService.findOrCreateItem(homeId, itemData.name);

...
export const addCommand = new Command("add")
  ...
  .action(async (name: string | undefined, options: Record<string, string>, command: Command) => {
    try {
      ...
      if (!name) {
        ...
        console.log(
          chalk.green(`✓ Added ${answers.quantity} ${answers.unit} of ${answers.name}`)
        );
      } else {
        // Direct mode with provided name
        console.log(chalk.green(`✓ Added ${options.quantity || 1} of ${name}`));
      }
    } catch (error) {

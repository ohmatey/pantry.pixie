...
describe("VALIDATION constants", () => {
  ...
  it("should have min < max for all length constraints", () => {
    expect(VALIDATION.USER.MIN_NAME_LENGTH).toBeLessThan(
      VALIDATION.USER.MAX_NAME_LENGTH,
    );
    expect(VALIDATION.USER.MIN_EMAIL_LENGTH).toBeLessThan(
      VALIDATION.USER.MAX_EMAIL_LENGTH,
    );
    expect(VALIDATION.ITEM.MIN_NAME_LENGTH).toBeLessThan(
      VALIDATION.ITEM.MAX_NAME_LENGTH,
    );
    expect(VALIDATION.LIST.MIN_NAME_LENGTH).toBeLessThan(
      VALIDATION.LIST.MAX_NAME_LENGTH,
    );
    expect(VALIDATION.CHAT.MIN_MESSAGE_LENGTH).toBeLessThan(
      VALIDATION.CHAT.MAX_MESSAGE_LENGTH,
    );
  });

  it("should have min quantity <= max quantity", () => {
    expect(VALIDATION.ITEM.MIN_QUANTITY).toBeLessThanOrEqual(
      VALIDATION.ITEM.MAX_QUANTITY,
    );
  });
});
